...
const Weather = () => {
  ...
  const [data, setData] = useState({ forecast: null, weatherData: null , pollution: null, dayForecast: null, historical: null});
    ...
    if (city !== "") {
      try {
        ...
        const forecasts = await axios.get(
          `https://pro.openweathermap.org/data/2.5/forecast/hourly?lat=${response.data.coord.lat}&lon=${response.data.coord.lon}&appid=51326c1d8dd29acfc399a1c78b2b21b7`
        );
          ...
          <div className='flex-row-farmadvice'>
            ...
            <div className='rain-advice'>
              <span className='rain-advice-5'>
                Feels Like: {data.weatherData.main.feels_like}°C. 
                {data.dayForecast.list[0].pop * 100}% chance of rain with 
                {data.dayForecast.list[1].rain}mm expected in the next 24 hours.
              </span>
